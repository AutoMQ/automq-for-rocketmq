...
public class S3StorageTest {
    ...
    public void testAppend() throws Exception {
        ...
        CommitSortedStreamTableObjectResponse resp = new CommitSortedStreamTableObjectResponse();
        Mockito.when(objectManager.commitSortedStreamTableObject(any())).thenReturn(CompletableFuture.completedFuture(resp));
        ...
        ArgumentCaptor<CommitSortedStreamTableObjectRequest> commitArg = ArgumentCaptor.forClass(CommitSortedStreamTableObjectRequest.class);
        verify(objectManager).commitSortedStreamTableObject(commitArg.capture());
        CommitSortedStreamTableObjectRequest commitReq = commitArg.getValue();
    ...
    public void testUploadWALObject_sequence() throws ExecutionException, InterruptedException, TimeoutException {
        ...
        List<CompletableFuture<CommitSortedStreamTableObjectResponse>> commitCfList = List.of(new CompletableFuture<>(), new CompletableFuture<>());
        AtomicInteger commitCfIndex = new AtomicInteger();
        Mockito.doAnswer(invocation -> commitCfList.get(commitCfIndex.getAndIncrement())).when(objectManager).commitSortedStreamTableObject(any());
        ...
        verify(objectManager, Mockito.timeout(1000).times(1)).commitSortedStreamTableObject(any());

        objectIdCfList.get(1).complete(2L);
        Thread.sleep(10);
        verify(objectManager, Mockito.times(1)).commitSortedStreamTableObject(any());

        commitCfList.get(0).complete(new CommitSortedStreamTableObjectResponse());
        verify(objectManager, Mockito.timeout(1000).times(2)).commitSortedStreamTableObject(any());
        commitCfList.get(1).complete(new CommitSortedStreamTableObjectResponse());
